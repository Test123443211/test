...
//            $xlsx = new SimpleXLSX('uploads/student_import.xlsx');
            ...
            $file = 'uploads/student_import.xlsx';
...
//                   echo "</pre><br/>";
                   ...
                   foreach($rowData as $r){  
                        $data['name'] = $r[0]; 
                        $data['birthday'] = date("d-m-Y", ($r[1] - 25569) * 86400);
                        $data['sex'] = $r[2]; 
                        $data['address'] = $r[3];
                        $data['phone'] = $r[4];
                        $data['email'] = $r[5];
                        $data['password'] = sha1($r[6]);
                        $data2['roll'] = $r[7];
                    //parent data ....
                        $data3['name'] = $r[8];
                        $data3['email'] = $r[9];
                        $data3['password'] = sha1($r[10]);
                        $data3['phone'] = $r[11];
                        $data3['address'] = $r[12];
                        $data3['profession'] = $r[13];  
                   }
                ...
                /* if($data3['phone'] !=''){
                  ...
                  $query = $this->db->get_where('parent',array('phone'=>$data3['phone']));
                  if($query){
                  foreach($query as $value){
                  echo $data3['name'] ." phone number ".$value;
                  }
                  }else{
                  echo "Not found";
                  }
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if($param1=='withdraw'){ 
            ...
            $status=0;
            $data['withdraw_date']      =  $this->input->post('withdraw');
            $date= date('Y-m-d',strtotime($data['withdraw_date']));
            $data['withdraw_detail'] = $this->input->post('description');
            $this->db->where('student_id', $param2);
            $this->db->update('student',  array('status'=> $status , 'withdraw_date' => $date ,'withdraw_detail' =>$data['withdraw_detail']  , 'withdraw_class_id'=>$param3 ));
   ...
   function ClassVoice_payment($class_id='', $running_year='')
    ...
    /********WITDRAW STUDNET********/
    function withdraw_student($class_id=''){
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url() . 'index.php?login', 'refresh');
        $page_data['page_title'] 	= get_phrase('Withdraw_Students'). " - ".get_phrase('class')." : ".$this->crud_model->get_class_name($class_id);
